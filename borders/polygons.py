...
class InsidePolygons():
    def __init__(self, polies):
        """
        Construct a precomputed object for fast inside/outside queries.
        polies: List[(N_i, 2) tensor (=points)].
        Points within any insides polygon will be classified as "inside", unless
        they're in any polygon of the "outsides".
        """
...
def circle(R, N, x=0.0, y=0.0):
    alpha = 2*math.pi / N
    return [[R*math.cos(alpha*i) + x, R*math.sin(alpha*i) + y] for i in range(N)]
    
def smiley():
    face = circle(1, 20)
    eye_left = circle(0.2, 20, -0.5, -0.25)
    eye_right = circle(0.2, 20, 0.5, -0.25)
    
    mouth = circle(0.70, 20, 0, 0.2)
    mouth = mouth[1:(len(mouth) // 2) ]

    return [torch.tensor(p) for p in (face, eye_left, eye_right, mouth)]
